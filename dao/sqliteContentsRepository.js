...
public IEnumerable<Content> GetContents()
{
    ...
    {
        return connection.Query<Content>(@"SELECT * FROM CONTENTS");
    }
}

public PagedResults<Content> GetTopContents(int startIndex, int numberOfItems)
{
    using (var connection = OpenDbConnection())
    {
        return new PagedResults<Content>(
            connection.Query<Content>(@"SELECT * FROM TOP_CONTENTS t INNER JOIN CONTENTS c ON t.IdContent = c.IdContent ORDER BY t.SalesOrder DESC LIMIT @count OFFSET @start", new
    {
        count = Math.Min(100, numberOfItems),
        start = startIndex
    }),
    connection.ExecuteScalar<int>(@"SELECT COUNT(*) FROM TOP_CONTENTS t INNER JOIN CONTENTS c ON t.IdContent = c.IdContent")
);
}
}
